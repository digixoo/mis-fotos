'use client'

import { useEffect, useState } from 'react'
import { useRouter } from 'next/navigation'
import { QRCodeSVG } from 'qrcode.react'

interface Props {
  nombre: string
  codigo: string
}

export default function SalaHeader({ nombre, codigo }: Props) {
  const router = useRouter()
  const [nombreInvitado, setNombreInvitado] = useState('')
  const [modalCompartir, setModalCompartir] = useState(false)
  const [copiado, setCopiado] = useState(false)
  const [urlSala, setUrlSala] = useState('')

  useEffect(() => {
    setNombreInvitado(localStorage.getItem('misfotos_nombre') ?? '')
    setUrlSala(`${window.location.origin}/sala/${codigo}`)
  }, [codigo])

  function handleSalir() {
    router.push('/')
  }

  async function handleCopiar() {
    await navigator.clipboard.writeText(urlSala)
    setCopiado(true)
    setTimeout(() => setCopiado(false), 2000)
  }

  function handleWhatsApp() {
    const texto = encodeURIComponent(
      `Te invito a ver y subir fotos de "${nombre}" 📷\nIngresá con el código *${codigo}*:\n${urlSala}`
    )
    window.open(`https://wa.me/?text=${texto}`, '_blank')
  }

  async function handleCompartir() {
    if (navigator.share) {
      try {
        await navigator.share({
          title: `MisFotos — ${nombre}`,
          text: `Te invito a ver y subir fotos de "${nombre}". Código: ${codigo}`,
          url: urlSala,
        })
        return
      } catch {
        // Si el usuario cancela, no hacer nada
      }
    }
    // Fallback: abrir modal
    setModalCompartir(true)
  }

  return (
    <>
      <header className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 sticky top-0 z-30">
        {/* Ícono */}
        <div className="w-9 h-9 rounded-full bg-rose-500 flex items-center justify-center flex-shrink-0">
          <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/>
            <circle cx="12" cy="13" r="4"/>
          </svg>
        </div>

        {/* Info sala */}
        <div className="flex-1 min-w-0">
          <h1 className="font-semibold text-gray-800 truncate leading-tight">{nombre}</h1>
          <p className="text-xs text-gray-400 leading-tight">
            {nombreInvitado ? `Hola, ${nombreInvitado}` : `Código: ${codigo}`}
          </p>
        </div>

        {/* Botón compartir */}
        <button
          onClick={handleCompartir}
          className="flex-shrink-0 flex items-center gap-1.5 text-sm text-gray-400 hover:text-rose-500 transition px-2 py-1 rounded-lg hover:bg-rose-50"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/>
            <line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/>
            <line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/>
          </svg>
          <span className="hidden sm:inline">Compartir</span>
        </button>

        {/* Botón salir */}
        <button
          onClick={handleSalir}
          className="flex-shrink-0 flex items-center gap-1.5 text-sm text-gray-400 hover:text-rose-500 transition px-2 py-1 rounded-lg hover:bg-rose-50"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/>
            <polyline points="16 17 21 12 16 7"/>
            <line x1="21" y1="12" x2="9" y2="12"/>
          </svg>
          <span className="hidden sm:inline">Salir</span>
        </button>
      </header>

      {/* Modal compartir */}
      {modalCompartir && (
        <div
          className="fixed inset-0 bg-black/70 z-50 flex items-end sm:items-center justify-center p-4"
          onClick={() => setModalCompartir(false)}
        >
          <div
            className="bg-white rounded-3xl w-full max-w-sm p-6 flex flex-col items-center gap-5"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="w-full flex items-center justify-between">
              <h2 className="font-semibold text-gray-800 text-lg">Compartir sala</h2>
              <button
                onClick={() => setModalCompartir(false)}
                className="text-gray-400 hover:text-gray-600 transition"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
                </svg>
              </button>
            </div>

            {/* QR */}
            <div className="bg-gray-50 rounded-2xl p-5 flex flex-col items-center gap-3">
              <QRCodeSVG value={urlSala} size={180} level="M" />
              <p className="text-xs text-gray-400 text-center">
                Mostrá este QR a quien tengas al lado
              </p>
            </div>

            {/* Código */}
            <div className="w-full bg-rose-50 rounded-xl px-4 py-2.5 flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-400">Código de sala</p>
                <p className="font-mono font-bold text-rose-500 text-lg tracking-widest">{codigo}</p>
              </div>
            </div>

            {/* Acciones */}
            <div className="w-full flex flex-col gap-2">
              <button
                onClick={handleWhatsApp}
                className="w-full h-12 rounded-xl bg-[#25D366] text-white font-semibold flex items-center justify-center gap-2 hover:bg-[#20b858] transition"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
                Compartir por WhatsApp
              </button>

              <button
                onClick={handleCopiar}
                className="w-full h-12 rounded-xl bg-gray-100 text-gray-700 font-semibold flex items-center justify-center gap-2 hover:bg-gray-200 transition"
              >
                {copiado ? (
                  <>
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                      <polyline points="20 6 9 17 4 12"/>
                    </svg>
                    <span className="text-emerald-600">¡Enlace copiado!</span>
                  </>
                ) : (
                  <>
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <rect x="9" y="9" width="13" height="13" rx="2" ry="2"/>
                      <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/>
                    </svg>
                    Copiar enlace
                  </>
                )}
              </button>
            </div>
          </div>
        </div>
      )}
    </>
  )
}
